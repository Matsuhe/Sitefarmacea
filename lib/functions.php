<?php
	...
	function get_categorias(){
		...
		$stmt = $pdo->prepare("SELECT categorias.id AS 'id', 
						        categorias.categoria AS 'categoria',
						        doencas.doenca AS 'doenca'
						        FROM 
						        categorias  
						        JOIN doencas ON categorias.doenca = doencas.id
		...
		if($total > 0){
			while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
				echo "
						<option value='{$dados['id']}'>{$dados['categoria']} - {$dados['doenca']}</option>
					";
	...
	function get_subcategorias(){
		...
		$stmt = $pdo->prepare("SELECT subcategorias.id AS 'id', 
		subcategorias.subcategoria AS 'subcategoria', 
        categorias.categoria AS 'categoria',
        doencas.doenca AS 'doenca'
		FROM 
		subcategorias JOIN categorias ON subcategorias.categoria = categorias.id 
		JOIN doencas ON categorias.doenca = doencas.id
		ORDER BY subcategorias.subcategoria ASC");
		$stmt->execute();
		$total = $stmt->rowCount();
		...
		if($total > 0){
			while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
				echo "
						<option value='{$dados['id']}'>{$dados['subcategoria']} (Categoria: {$dados['categoria']}) - Doença: {$dados['doenca']}</option>
					";

			}
		}else{
			echo alerta("danger", "É necessário ter pelo menos uma subcategoria antes de criar publicações. Crie uma subcategoria para continuar.");
			exit();
		}
	}
	...
	function get_doencas(){
		...
		$stmt = $pdo->prepare("SELECT * FROM doencas ORDER BY doenca ASC");
		$stmt->execute();
		$total = $stmt->rowCount();
		...
		if($total > 0){
			while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
				echo "
						<option value='{$dados['id']}'>{$dados['doenca']}</option>
					";

			}
		}else{
			echo alerta("danger", "É necessário ter pelo menos uma doenca antes de criar publicações. Crie uma doenca para continuar.");
			exit();
		}
	}
	...
	function getNomeSubCategoria($id){
		$pdo = pdo();
	...
	function getPostsAdmin(){
		...
		$stmt = $pdo->prepare("SELECT posts.id AS 'id', posts.titulo AS 'Titulo', posts.subtitulo AS 'subtitulo', subcategorias.subcategoria AS 'Subcategoria', categorias.categoria AS 'Categoria', doencas.doenca AS 'Doenca' FROM posts JOIN subcategorias ON posts.subcategoria = subcategorias.id JOIN categorias ON subcategorias.categoria = categorias.id JOIN doencas ON categorias.doenca = doencas.id ORDER BY posts.id DESC");
		$stmt->execute();
		$total = $stmt->rowCount();
		...
		if($total > 0):
			while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
				echo "<tr>
  <td style='width:1px'>{$dados['Titulo']}</td>
  <td style='width: 1px'>{$dados['Doenca']}</td>
  <td style='width: 1px'>{$dados['Categoria']}</td>
  <td td style='width: 1px'>{$dados['Subcategoria']}</td>
  <td><button id='btnGroupDrop1' type='button' class='btn btn-secondary dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>Gerenciar</button>
	...
	function addDoenca(){
		if(isset($_POST['env']) && $_POST['env'] == "doen"){
			$pdo = pdo();

			$stmt = $pdo->prepare("INSERT INTO doencas (doenca) VALUES (:doenca)");
			$stmt->execute([':doenca' => $_POST['doenca']]);
			...
			if($total > 0){
				...
				alerta("danger", "Erro ao cadastrar a doenca");
			}
		}
	}
	...
	function addCategoria(){
		if(isset($_POST['env']) && $_POST['env'] == "cat"){
			...
			$stmt = $pdo->prepare("INSERT INTO categorias (categoria, doenca) VALUES (:categoria, :doenca)");
	...
	function addSubCategoria(){
		if(isset($_POST['env']) && $_POST['env'] == "subcat"){
			$pdo = pdo();

			$stmt = $pdo->prepare("INSERT INTO subcategorias (subcategoria, categoria) VALUES (:subcategoria, :categoria)");
			$stmt->execute([':subcategoria' => $_POST['subcategoria'],
							':categoria' => $_POST['categoria']]);

			$total = $stmt->rowCount();

			if($total > 0){
				alerta("success", "Sub-Categoria cadastrada com sucesso!");
			}else{
				alerta("danger", "Erro ao cadastrar categoria");
			}
		}
...
function getDoencasMenu(){
		...
		if($total > 0){
			while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
				echo "<li>{$dados['doenca']} <a href='admin/deletar-doenca/{$dados['id']}' class='btn btn-danger btn-sm float-right'>Deletar</a></li>";
			}
		}
	}
	...
	function getCategoriasMenu(){
		...
		if($total > 0){
			while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
				echo "<li>{$dados['categoria']} - {$dados['doenca']} <a href='admin/deletar-categoria/{$dados['id']}' class='btn btn-danger btn-sm float-right'>Deletar</a></li>";
			}
		}
	}


	function getSubCategoriasMenu(){
		$pdo = pdo();
		...
		if($total > 0){
			while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
				echo "<li>{$dados['subcategoria']} - {$dados['categoria']} - {$dados['doenca']}</li> <a href='admin/deletar-subcategoria/{$dados['id']}' class='btn btn-danger btn-sm float-right'>Deletar</a><br><br>";
			}
		}
	}
...
lauchModal($dados['id'], $dados['nome'], $dados['comentario']);
	...
	function getPosts(){
		...
		$stmt = $pdo->prepare("SELECT posts.id AS 'id', 
			posts.titulo AS 'titulo', 
			posts.subtitulo AS 'subtitulo', 
			posts.subcategoria AS 'sub_id',
			posts.postagem AS 'postagem', 
			posts.id_postador AS 'id_postador', 
			posts.visualizacoes AS 'visualizacoes', 
			posts.data as 'data', 
			subcategorias.subcategoria AS 'subcategoria', 
			categorias.categoria AS 'categoria', 
			doencas.doenca AS 'doenca' 
			FROM posts JOIN subcategorias ON posts.subcategoria = subcategorias.id JOIN categorias ON subcategorias.categoria = categorias.id JOIN doencas ON categorias.doenca = doencas.id ORDER BY posts.id DESC limit $inicio,$maximo");
		$stmt->execute();
  ...
  <div class='infos'>
    ...
    <i class='fas fa-tag'></i> <span class='badge badge-primary'>".($dados['subcategoria'])."</span> |
    <i class='fas fa-tag'></i> <span class='badge badge-success'>".($dados['categoria'])."</span> |
    <i class='fas fa-tag'></i> <span class='badge badge-danger'>".($dados['doenca'])."</span> |
  ...
  <div class='infos'>
	...
	function getPostsFromBusca(){
		...
		$stmt = $pdo->prepare("SELECT posts.id AS 'id', 
            ...
            categorias.categoria AS 'categoria', 
            doencas.doenca AS 'doenca' 
            FROM posts JOIN subcategorias ON posts.subcategoria = subcategorias.id JOIN categorias ON subcategorias.categoria = categorias.id JOIN doencas ON categorias.doenca = doencas.id 
            WHERE titulo LIKE :titulo 
            OR postagem LIKE :postagem 
            ORDER BY id DESC ");
		$stmt->execute([':titulo' => $busca, 
						':postagem' => $busca]);
  ...
  <div class='infos'>
	...
	function getCompletePost(){
		...
		$stmt = $pdo->prepare("SELECT posts.id AS 'id', 
			...
			posts.visualizacoes AS 'visualizacoes', 
			posts.data as 'data',
			posts.imagem as 'imagem', 
			subcategorias.subcategoria AS 'subcategoria', 
			categorias.categoria AS 'categoria', 
			doencas.doenca AS 'doenca' 
			FROM posts JOIN subcategorias ON posts.subcategoria = subcategorias.id JOIN categorias ON subcategorias.categoria = categorias.id JOIN doencas ON categorias.doenca = doencas.id WHERE subtitulo = :subtitulo");
  ...
  <div class='row'>
    <div class='col-sm-2'>
      <img src='images/template/nophoto.png' class='-fluid'>
    </div>
    <div class='col-sm'>
      <div class='card'>
